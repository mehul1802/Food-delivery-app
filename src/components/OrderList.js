                    ...
                    <div className="pt-2">
                      {order.order_items && order.order_items.map(item => (
                        <div className="d-flex product-list py-3 px-4" key={item.name}>
                          <div className="w-25">
                            <img alt="product" className="w-100" src="https://www.yourmomhatesthis.com/images/2016/12/Pizza-Free-PNG-Image.png" />
                          </div>
                          <div className="w-75 pl-3 pt-2 d-flex justify-content-between">
                            <div>
                              <h2 className="text-primary font-weight-bold">{item.name}</h2>
                              <div className="font-small text-light-grey d-flex order-qty">
                                <div>quantity: <span className="text-dark">{item.quantity}</span></div>
                                <div className="ml-2 unit-price">Unit price: <span className="text-dark">{formatPrice(item.unit_price)}</span></div>
                              </div>
